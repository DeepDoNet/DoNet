...
class DuringTrainAMODALCOCOeval (COCOeval):
        ...
        if not len(ious) == 0:
            ...
            if p.iouType == 'segm':
                ...
                if len(gt) != 0 and len(dt) != 0:
                    ...
                    PLIST, RLIST = [1 for i in range(28)], [
                        1 for i in range(28)]
                mdsc, mtpr, FNR, FDR = self.caclulateMetrics(ious_for_AJI,
        ...
        def _summarizeSegm():
            ...
            for catId, cat in enumerate(self._paramsEval.catIds):
                ...
                if cat == 0:
                    ...
                    cat_name = 'Nuclei'
                print("AJI", cat_name, AJI[cat])
                print("F1", cat_name, F1_score[cat])
                print("DSC", cat_name, DSC[cat])
            return AJI
        if not self.eval:
