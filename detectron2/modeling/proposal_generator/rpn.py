...
class RPN(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None, with_attention = False, unlabeled = False):
        """
        Args:
        ...
        if self.training:
            if with_attention:
                losses = {k+'_with_attention': v * self.loss_weight for k, v in outputs.losses().items()}
            else:
        ...
        with torch.no_grad():
            ...
            proposals = find_top_rpn_proposals(
                ...
                100 if unlabeled else self.post_nms_topk[self.training],
